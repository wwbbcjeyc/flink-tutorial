...
//        env.enableCheckpointing(3000L,CheckpointingMode.AT_LEAST_ONCE);
        ...
        env.getCheckpointConfig().setMaxConcurrentCheckpoints(2);            // 异步ck，同时有几个ck在执行 异步设置
        env.getCheckpointConfig().setMinPauseBetweenCheckpoints(500L);        // 上一个ck结束后，到下一个ck开启，最小间隔多久 同步设置
